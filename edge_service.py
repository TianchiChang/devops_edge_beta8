...
def device_control_update(device_id, action):
    # 完成 <登陆设备管理> 与 <中心云设备同步>
    try:
        if action == "login":
        ...
        request = requests.post("http://{}:{}/{}".format(cloud_host, cloud_port, "device_async"), data=json.dumps({"device_id": device_id, "action": action}))
...
async def device_login(request, ws):
    try:
        ...
        print(auth_resp)
        if json.loads(auth_resp)["method"]!="auth":
            return text("Unauthorized", status=401)

        try: 
            target = json.loads(auth_resp)["params"][1]
            device_pwd = saltdec(target, pwd)
            device_id = json.loads(auth_resp)["params"][0]
        except:
            traceback.print_exc()
        ...
        if sql_device_service_exist(device_id): 
            if device_pwd==device_id: 
                device_control_update(device_id=device_id, action="login")
...
#                ws.send(saltenc(new_token))
        ...
        device_control_update(device_id=device_id, action="logout")
            ...
            if auth:
                ...
                while True:
                    ...
                    if resp["method"] == "register":
                        ...
                        suc = sql_device_service_insert(device_id, device_host, device_port)

                    elif resp["method"] == "delete":
                        device_id = resp["params"][0]
                        suc = sql_device_service_delete(device_id)
...
def start():
    app_run(edge_host, edge_port, access_log=False, debug=True, workers=2)
